...
return array (
	  ...
	  'LOAD_EXT_CONFIG' => 'db,core,domain,html,copy',
	  'LOAD_EXT_FILE' => 'tag',
   ...
   'TMPL_STRIP_SPACE' =>true,  //是否去除html中的空格
   //'TAGLIB_LOAD'=>true,
   ...
   'TAGLIB_BUILD_IN' => 'youDian,cx',
	...
	'URL_ROUTE_RULES'  => array(
			...
			//'/app\/tpl\/.+\.html$/i'=>'home/index/index',
			
			//wap网站路由规则
			'/^wap[\/]?(en|cn)?$/i' => 'wap/index/index?l=:1', //用于Wap频道,匹配：www.csyoudian.com/wap
			'/^wap\/(en\/|cn\/)?d(\d+)$/i'=>'wap/channel/index?id=order&infoid=:2&l=:1', //d:匹配在线订购，参数id,infoid必须为小写
			'/^wap\/(en\/|cn\/)?info\/([\w-]+)$/i'=>'wap/info/read?id=:2&l=:1',  //信息阅读
			'/^wap\/(en\/|cn\/)?r(\d+)$/i'=>'wap/channel/index?id=resume&jobid=:2&l=:1', //匹配投递简历，参数id,infoid必须为小写
			'/^wap\/(en\/|cn\/)?([\w-]+)$/i' => 'wap/channel/index?id=:2&l=:1', //用于不带参数的频道
			
			//Home分组路由规则
			'/^(en|cn)$/i' => 'index/index?l=:1', //用于不带参数的频道
			'/^(en\/|cn\/)?info\/([\w-]+)$/i'=>'info/read?id=:2&l=:1',  //信息阅读
