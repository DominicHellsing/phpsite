...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        $LoginAdmin = stripos($_SERVER['PATH_INFO'], '/admin'); 
        if(C('APP_SUB_DOMAIN_DEPLOY') && 0 !== $LoginAdmin) {
        	/*
            ...
            */
        	...
        	//修改成能绑定一级域名==============================================
        	//判断手机网站是否存在，若不存在，则取消手机网站域名绑定
        	$t = include_once CONF_PATH.'Wap/config.php';
        	$HasWap = file_exists(TMPL_PATH.'Wap/'.$t['DEFAULT_THEME'].'/template.xml');
        	if( $HasWap ){
	        	//修改后不再支持泛域名
	        	$rules = array_change_key_case(C('APP_SUB_DOMAIN_RULES'));
